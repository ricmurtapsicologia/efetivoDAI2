...
IMAGE = "https://ricmurtapsicologia.github.io/efetivoDAI2/assets/img/banner-cbmmg.jpg"
UAS = [
...
def request(url: str, ua: str, timeout: int = 20):
...
def main() -> int:
    ...
    for _ in range(30):
        ...
        print(f"FAIL | deploy não convergiu para OG local | {last_error or 'imagem OG local ausente'}")
        return 1
    ...
    results.append(check("06 og:image same-origin local", og_image == IMAGE, og_image))
    results.append(check("07 og:image:secure_url local", og_secure == IMAGE, og_secure))
    results.append(check("08 og:image MIME declarado", og_type == "image/jpeg", og_type))
    results.append(check("09 twitter:image alinhada", twitter_image == IMAGE, twitter_image))

    for ua in UAS:
        try:
            with request(PAGE + f"?ua_probe={int(time.time())}", ua) as response:
                crawler_html = response.read().decode("utf-8", errors="replace")
                page_ok = int(response.status) == 200 and IMAGE in crawler_html
            results.append(check(f"10 página entregue ao crawler {ua.split('/')[0]}", page_ok))
        except Exception as exc:
            results.append(check(f"10 página entregue ao crawler {ua.split('/')[0]}", False, str(exc)))
    ...
    try:
        ...
        results.append(check("11 imagem pública HTTP 200", status == 200, str(status)))
        results.append(check("12 imagem servida como JPEG", content_type in {"image/jpeg", "image/jpg"}, content_type))
        results.append(check("13 imagem possui assinatura JPEG", data[:2] == b"\xff\xd8" and data[-2:] == b"\xff\xd9", f"{len(data)} bytes"))
        results.append(check("14 imagem em faixa segura de tamanho", 10_000 <= len(data) <= 1_500_000, f"{len(data)} bytes"))
    except Exception as exc:
        results.append(check("11-14 imagem pública acessível", False, str(exc)))
    ...
    if passed != total:
        return 1
    print("SHARE PREVIEW: APROVADO — OG local, same-origin e acessível a crawlers sociais")
    return 0
