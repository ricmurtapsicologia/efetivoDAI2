...
def main() -> int:
    ...
    with sync_playwright() as p:
        ...
        check(results, 'UI 02 · hero corporativo presente', 'photo-1521737711867-e3b97375f902' in page.locator('header.banner').evaluate("el => getComputedStyle(el).backgroundImage"))
        ...
        check(results, 'E2E 08 · botão WhatsApp existe em cada militar do modal', page.locator('#militares-list .whatsapp-action').count() == page.locator('#militares-list .modal-military-item').count())
        page.keyboard.press('Escape')
        check(results, 'E2E 09 · Escape fecha modal', not page.locator('#modal').is_visible())
        ...
        check(results, 'E2E 10 · pesquisa nominal funciona', 'Silvana Tiengo' in search_text and 'AFAS' in search_text, search_text.replace('\n',' | ')[:220])
        ...
        check(results, 'E2E 12 · pesquisa de claro por P/G funciona', bool(claro_text.strip()) and 'Claro' in claro_text, claro_text.replace('\n',' | ')[:220])

        page.locator('#showListBtn').click()
        check(results, 'E2E 13 · lista DDQOD expande', page.locator('#militaryList').is_visible())
        ...
        check(results, 'E2E 14 · modal jurídico abre', page.locator('#normasModal').is_visible())
        ...
        check(results, 'E2E 15 · TPB abre', page.locator('#tpbModal').is_visible())
        ...
        check(results, 'QA 06 · KPIs TPB conferem', all(value in tpb_text for value in ['85','69','16','9','7','15']), tpb_text.replace('\n',' | ')[:260])
