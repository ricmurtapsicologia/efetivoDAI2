#!/usr/bin/env python3
"""30-point post-deploy smoke: layout, invariantes, privacidade, a11y e UX estrutural."""
...
def fetch(path, timeout=20):
    ...
    sep = "&" if "?" in url else "?"
    req = urllib.request.Request(url + sep + f"smoke={int(time.time())}", headers={"User-Agent":"DAI2-live-smoke/3.0"})
...
def main():
    ...
    for _ in range(30):
        try:
            status, index = fetch("")
            if status == 200 and 'assets/js/app.js?v=2.3.0' in index and 'data/tpb.js?v=2026.09.06-public' in index and 'class="banner"' in index:
                break
        except Exception as exc:
            last = exc
        time.sleep(5)
    else:
        print(f"FAIL: Pages não convergiu para interface 2.3.0: {last}")
        return 1
    ...
    app_status, app = fetch("assets/js/app.js?v=2.3.0")
    css_status, css = fetch("assets/css/main.css?v=2.1.0")

    meta = assignment(data, "DAI2_META")
    ...
    tests = [
      ...
      ("10 CSS 2.1 carregado", css_status == 200 and 'assets/css/main.css?v=2.1.0' in index),
      ("11 app 2.3 carregado", app_status == 200 and 'assets/js/app.js?v=2.3.0' in index),
      ...
      ("16 ações diretas de contato removidas da camada pública", 'wa.me' not in frontend and 'mailto:' not in frontend and 'Enviar WhatsApp' not in frontend and 'Enviar e-mail' not in frontend),
      ("17 aniversários não carregados e aviso de privacidade presente", 'DAI2_BIRTHDAYS' not in frontend and 'Proteção de dados' in index and 'Telefones, e-mails, aniversários' in app),
      ...
      ("24 TPB agregado carregado", tpb_status == 200 and not tpb_rows and 'id="tpbToggleBtn"' in index and 'id="tpbModal"' in index),
      ("25 corte TPB = 03/09/2026", tpb_meta.get("cutoff") == "03/09/2026"),
      ("26 escopo TPB agregado = 85", int(expected.get("scope",-1)) == 85),
      ("27 TPB feito agregado = 69", int(expected.get("done",-1)) == 69),
      ("28 não feito = 16 / dispensas = 9", int(expected.get("notDone",-1)) == 16 and int(expected.get("dispensed",-1)) == 9),
      ("29 exigem ação = 7 / sem data consolidada = 15", int(expected.get("actionable",-1)) == 7 and int(expected.get("dateToConfirm",-1)) == 15),
      ("30 a11y/UX/privacidade estrutural", not any(term in frontend for term in BANNED) and '.innerHTML' not in app and 'prefers-reduced-motion' in css and 'viewport-fit=cover' in index and 'role="dialog"' in index and 'class="skip-link"' in index and 'function trapFocus' in app and 'aria-expanded="false"' in index and not re.search(r'<div class="numbers">\s*Previsto:', index)),
    ]
    ...
    if failed:
        ...
        return 1
    print("SMOKE PÚBLICO: APROVADO 30/30 — layout preservado, dados governados e camada pública sem contatos pessoais")
    return 0
